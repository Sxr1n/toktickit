...
| D-02 | Passwords are hashed with `bcryptjs` (pure JavaScript), not native `bcrypt`, at cost factor 10. | Avoids native-module build issues on Windows dev machines, already a recurring friction point in this repo's history (Prisma's own native engine had to be worked around similarly). Cost factor 10 (bcrypt's library default) documented here explicitly, per PR #34 review feedback asking for it. Login rate-limiting was also raised in that review and is out of scope for this course lab, consistent with BR-32's other explicitly-out-of-scope items. |
